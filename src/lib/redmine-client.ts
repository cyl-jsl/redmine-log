...
export class RedmineClient {
  ...
  private async get<T>(
    ...
    try {
      ...
      throw new Error(
        `無法連線到 Redmine (${this.baseUrl})：${err instanceof Error ? err.message : String(err)}`,
        { cause: err },
      );
    }
    if (!res.ok)
  ...
  async listIssues(
    ...
    return (await this.get<{ issues: RedmineIssue[] }>("/issues.json", params))
      .issues;
  }
  ...
  async listTimeEntries(
    ...
    return (
      await this.get<{ time_entries: RedmineTimeEntry[] }>(
        "/time_entries.json",
        params,
      )
    ).time_entries;
